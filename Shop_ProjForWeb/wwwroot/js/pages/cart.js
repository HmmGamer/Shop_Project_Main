...
class CartPage {
        ...
        try {
            ...
            container.innerHTML = `
                <div class="error-message">
                    ...
                    <p>${error.message}</p>
                    <button class="btn btn-primary" onclick="window.app.router.navigate('cart')">
                        Try Again
                    </button>
                </div>
            `;
        ...
        return `
            <div class="cart-items-section">
                ...
                <div class="cart-actions">
                    <button class="btn btn-outline clear-cart-btn" id="clear-cart-btn">
                        Clear Cart
                    </button>
                    <button class="btn btn-primary continue-shopping-btn" onclick="window.app.router.navigate('catalog')">
                        Continue Shopping
                    </button>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    ${isVip ? `
                        <div class="summary-line vip-line">
                            ...
                            <span class="vip-discount">-$${(total * 0.05).toFixed(2)}</span>
                        </div>
                    ` : ''}
                    <div class="summary-line total-line">
                        <span><strong>Total:</strong></span>
                        <span><strong>$${(isVip ? total * 0.95 : total).toFixed(2)}</strong></span>
                    </div>
                    ${isVip ? `
                        <div class="vip-badge-container">
                            <span class="vip-badge">VIP Member - 5% Off!</span>
                        </div>
                    ` : ''}
                </div>
                <div class="card-footer">
                    <button class="btn btn-primary btn-lg checkout-btn" id="checkout-btn" 
                            ${this.isProcessingCheckout ? 'disabled' : ''}>
    ...
    updateQuantity(productId, newQuantity) {
        if (newQuantity < 1) {
            ...
            if (input) {
                input.value = this.stateManager.getCartItemQuantity(productId);
            }
        } else {
            ...
            modalContent,
            {
                buttons: [
                    ...
                    },
                    {
                        text: 'Clear Cart',
                        class: 'btn-error',
                        onclick: 'window.app.pages.cart.clearCart(); window.app.closeModal();'
                    }
                ]
            }
        );
    ...
    showLoginPrompt() {
        const modalContent = `
            <div class="login-prompt">
                ...
                <p>Would you like to create an account or log in now?</p>
            </div>
        `;
            ...
            modalContent,
            {
                buttons: [
                    ...
                    },
                    {
                        text: 'Go to Account',
                        class: 'btn-primary',
                        onclick: 'window.app.closeModal(); window.app.router.navigate(\'account\');'
                    }
                ]
            }
        );
    ...
    showOrderSuccess(order) {
        const modalContent = `
            <div class="order-success">
                ...
                <p>Your order #${order.orderId.slice(-8)} has been created.</p>
                <p><strong>Total: $${order.totalPrice.toFixed(2)}</strong></p>
                <p class="text-muted">You can view your order details in your account.</p>
            </div>
        `;
            ...
            modalContent,
            {
                buttons: [
                    ...
                    },
                    {
                        ...
                        class: 'btn-outline',
                        onclick: 'window.app.closeModal(); window.app.router.navigate(\'catalog\');'
                    }
                ]
            }
        );
    ...
    updateCartDisplay() {
        ...
        if (cartDescription) {
            cartDescription.textContent = this.cart.length === 0 
                ? 'Your cart is empty' 
                : `${this.getCartItemCount()} item${this.getCartItemCount() !== 1 ? 's' : ''} in your cart`;
        }
    }
    ...
    calculateDiscountAmount() {
        return this.cart.reduce((total, item) => {
            if (item.discountPercent > 0) {
                const itemDiscount = (item.basePrice * item.discountPercent / 100) * item.quantity;
    ...
    getCartTotal() {
        return this.stateManager.getCartTotal();
    }

    getCartItemCount() {
        return this.stateManager.getCartItemCount();
    }
